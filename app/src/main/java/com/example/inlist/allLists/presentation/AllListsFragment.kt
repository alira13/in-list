...
class AllListsFragment : Fragment() {
    ...
    override fun onCreateView(
        ...
        val allListsViewModel =
            ViewModelProvider(this).get(AllListsViewModel::class.java)

        _binding = FragmentAllListsBinding.inflate(inflater, container, false)
        val root: View = binding.root

        val textView: TextView = binding.textAllLists
        allListsViewModel.text.observe(viewLifecycleOwner) {
            textView.text = it
        }
        return root
    }
